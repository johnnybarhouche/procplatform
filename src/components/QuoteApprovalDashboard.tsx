...
export default function QuoteApprovalDashboard({ userRole: _userRole }: QuoteApprovalDashboardProps) {
  ...
  const [activeApproval, setActiveApproval] = useState<QuoteApproval | null>(null);
  const [activeLoading, setActiveLoading] = useState(false);
  const [overlayError, setOverlayError] = useState<string | null>(null);
  const [toastMessage, setToastMessage] = useState<string | null>(null);

  useEffect(() => {
    void fetchQuoteApprovals();
  }, []);
  ...
  const openApproval = async (approvalId: string) => {
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(data.error || "Unable to load quote approval.");
      }

      const data = (await response.json()) as QuoteApproval;
      setActiveApproval(data);
    } catch (error) {
      setOverlayError(error instanceof Error ? error.message : "Unable to load quote approval.");
    } finally {
  ...
  const sortedApprovals = [...filteredApprovals].sort((a, b) => {
    switch (sortBy) {
      ...
      case "project":
        return a.quote_pack.rfq.material_request.project_name.localeCompare(
          b.quote_pack.rfq.material_request.project_name
        );
      default:
        return 0;
    }
  ...
  return (
    <>
      ...
      >
        {toastMessage && (
          <div className="mb-4 rounded-lg border border-status-success/40 bg-status-success/10 px-4 py-3 text-sm text-status-success">
            {toastMessage}
          </div>
        )}

        {overlayError && (
          <div className="mb-4 rounded-lg border border-status-danger/40 bg-status-danger/10 px-4 py-3 text-sm text-status-danger">
            {overlayError}
          </div>
        )}
                ...
                <div key={approval.id} className="px-6 py-4">
                  <div className="flex items-center justify-between">
                    <div className="flex-1">
                      ...
                      <div className="mt-2">
                        <p className="text-sm text-brand-text/70">
                          {approval.quote_pack.quotes.length} quotes received • Total Value: {approval.quote_pack.quotes
