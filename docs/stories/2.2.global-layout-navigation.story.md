...
## Acceptance Criteria
1. Implement `AppLayout` with Deep Blue top bar, white page surface, and responsive nav (desktop sidebar + mobile bottom nav), consuming tokens from Story 1.2; keep the top bar and desktop sidebar fully affixed (no drift during scroll) and arrange header content as company logo + active project name on the left and, left→right, role indicator (bold/non-interactive), notifications bell, and user initials avatar opening the profile menu—aligned to a single baseline. [Source: docs/front-end.md#2-global-layout; docs/uiux/design-tokens.md]
...
- [x] Task 1: Build layout primitives (AC: 1, 2)
  - [x] Subtask 1.1: Create `src/components/layout/AppLayout`, `TopBar`, `SideNav`, `BottomNav` utilities using design tokens and pull the logo/project name from admin configuration data. [Source: docs/uiux/design-tokens.md]
...
| 2025-01-21 | 0.5     | Updated TopBar layout per requirements, added control panel, cleaned up code | James |
...
- **Updated TopBar Layout**: Removed interactive project/role selectors from TopBar and moved them to main content area as per requirements.
- **Enhanced TopBar**: Implemented proper layout with company logo + project name on left, role indicator + notifications + user avatar on right.
- **Added Control Panel**: Created project and role selectors in main content area for better user experience.
- **Cleaned Up Code**: Removed unused parameters and variables to resolve lint warnings.

### File List
**Modified Files:**
- `src/components/layout/AppLayout.tsx` - Removed unused parameters, updated interface
- `src/components/layout/TopBar.tsx` - Updated layout to match requirements, removed interactive selectors
- `src/app/page.tsx` - Added project and role control panel to main content area
- `docs/stories/2.2.global-layout-navigation.story.md` - Updated completion status

**Existing Files:**
- `src/components/layout/SideNav.tsx` - Sidebar navigation component
- `src/components/layout/BottomNav.tsx` - Mobile bottom navigation
- `src/components/layout/navConfig.tsx` - Navigation configuration
- `src/app/ui-kit/page.tsx` - UI Kit showcase page
- `src/app/globals.css` - Global styles
- `docs/uiux/ui-kit-notes.md` - UI Kit documentation
